...
begin
  if not exists (select 1 from pg_type where typname = 'ai_provider') then
    create type public.ai_provider as enum ('openai','anthropic','google','groq','together','openrouter','huggingface','custom');
  end if;
...
create trigger ai_keys_set_updated_at before update on public.user_ai_keys for each row execute function public.set_updated_at();

drop trigger if exists ai_prefs_set_updated_at on public.user_ai_preferences;
create trigger ai_prefs_set_updated_at before update on public.user_ai_preferences for each row execute function public.set_updated_at();

alter table public.user_ai_keys enable row level security;
alter table public.user_ai_preferences enable row level security;

drop policy if exists "ai_keys own" on public.user_ai_keys;
create policy "ai_keys own" on public.user_ai_keys for all to authenticated using (user_id = auth.uid()) with check (user_id = auth.uid());

drop policy if exists "ai_prefs own" on public.user_ai_preferences;
create policy "ai_prefs own" on public.user_ai_preferences for all to authenticated using (user_id = auth.uid()) with check (user_id = auth.uid());
